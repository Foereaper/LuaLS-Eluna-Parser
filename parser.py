...
def extract_arguments(soup):
    ...
    for dl in dl_elements:
        ...
        argument_name = dt_element.text.replace(argument_type, '').strip().split(" ", 1)[0]
        ...
        arguments_list.append({'type': argument_type, 'name': argument_name, 'description': argument_description})
...
def write_lua_stub(class_name, method_string, arguments_list, return_values_list, output_directory):
    with open(os.path.join(output_directory, class_name + ".lua"), "a") as lua_file:
        ...
        for argument in arguments_list:
            lua_file.write("---@param " + argument['name'] + " " + argument['type'] + " : " + argument['description'] + "\n")
...
def main():
    # Check if command-line arguments are provided properly
    if len(sys.argv) != 3:
